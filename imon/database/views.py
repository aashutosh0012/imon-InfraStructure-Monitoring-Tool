...
#         context = super().get_context_data(**kwargs)
#         context['fields'] = [ field.name for field in Server._meta.get_fields() if (field.name not in ('id','slug','servermetrics','diskvolumes','cpumemoryusagehistory','database'))]
#         return context
...
#     context = {'servers_list' : servers_list }
#     context['fields'] = [ field.name for field in Server._meta.get_fields() if (field.name not in ('id','slug'))] 
#     return render(request,template, context)
